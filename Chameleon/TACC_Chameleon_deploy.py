...
brief     Installs and deploys LDM7 on the Chameleon@TACC.
"""
...
def init_config():
    ...
    if iface == '192.168.1.1':	
        put('~/Workspace/Exp/std_data/1h_std', '/home/cc', mode=0664)
        config_str = ('MULTICAST ANY 224.0.0.1:38800 1 10.10.1.18\n'
                      'ALLOW ANY ^.*$\nEXEC \"insert.sh\"'
                      '\nEXEC \"cpu_mon.sh\"\nEXEC \"tc_mon.sh\"')
        run('sudo route add 224.0.0.1 dev %s' % IFACE_NAME, quiet=True)
        run('regutil -s 5G /queue/size', quiet=True)
    else:
        config_str = 'RECEIVE ANY 192.168.1.1:38800 ' + iface
...
def fetch_log():
    ...
    if iface == '192.168.1.1':
        with settings(sudo_user='cc'), cd('/home/cc'):
            ...
            get('cpu_measure.log', '~/Workspace/Exp/LOG/')
            get('bandwidth.log', '~/Workspace/Exp/LOG/')
            get('tc_mon.log', '~/Workspace/Exp/LOG/')
	    run('rm tc_mon.log bandwidth.log cpu_measure.log')	
    else:
